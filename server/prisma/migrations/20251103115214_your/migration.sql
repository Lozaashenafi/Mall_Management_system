/*
  ...
  - The values [Failedte] on the enum `Payment_status` will be removed. If these variants are still used in the database, this will fail.
...
-- AlterTable
ALTER TABLE `notification` MODIFY `type` ENUM('Invoice', 'Maintenance', 'PaymentReminder', 'RenewalReminder', 'OverduePayment', 'PaymentConfirmation', 'PaymentRequest', 'UtilityAlert', 'SystemAlert') NOT NULL;

-- AlterTable
ALTER TABLE `payment` MODIFY `status` ENUM('Confirmed', 'Pending', 'Failed') NOT NULL DEFAULT 'Confirmed';
