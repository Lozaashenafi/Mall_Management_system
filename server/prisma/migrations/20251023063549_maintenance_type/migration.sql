...
-- AlterTable
ALTER TABLE `notification` MODIFY `type` ENUM('Invoice', 'Maintenance', 'PaymentReminder', 'RenewalReminder', 'SystemAlert') NOT NULL;
...
-- AddForeignKey
ALTER TABLE `Tenant` ADD CONSTRAINT `Tenant_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `User`(`userId`) ON DELETE SET NULL ON UPDATE CASCADE;
