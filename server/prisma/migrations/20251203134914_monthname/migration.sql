/*
  ...
  - You are about to drop the column `generated` on the `utilitycharge` table. All the data in the column will be lost.
  - You are about to drop the column `month` on the `utilitycharge` table. All the data in the column will be lost.
  - Added the required column `monthName` to the `Budget` table without a default value. This is not possible if the table is not empty.
...
-- DropIndex
DROP INDEX `ExitRequest_rentId_fkey` ON `exitrequest`;

-- DropIndex
DROP INDEX `ExitRequest_tenantId_fkey` ON `exitrequest`;

-- DropIndex
DROP INDEX `ExitRequest_verifiedBy_fkey` ON `exitrequest`;

-- DropIndex
DROP INDEX `ExitRequestItem_exitRequestId_fkey` ON `exitrequestitem`;
...
-- DropIndex
DROP INDEX `PaymentRequest_bankAccountId_fkey` ON `paymentrequest`;
...
-- DropIndex
DROP INDEX `UtilityInvoice_paymentId_fkey` ON `utilityinvoice`;

-- DropIndex
DROP INDEX `UtilityInvoice_paymentRequestId_fkey` ON `utilityinvoice`;
...
ALTER TABLE `budget` DROP COLUMN `month`,
    ADD COLUMN `monthName` INTEGER NOT NULL;

-- AlterTable
ALTER TABLE `utilitycharge` DROP COLUMN `generated`,
    DROP COLUMN `month`,
    ADD COLUMN `isGenerated` BOOLEAN NULL DEFAULT false,
    ADD COLUMN `monthName` VARCHAR(191) NULL;
