/*
  ...
  - Added the required column `categoryId` to the `MaintenanceSchedule` table without a default value. This is not possible if the table is not empty.
  - Added the required column `startDate` to the `MaintenanceSchedule` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `maintenanceschedule` DROP COLUMN `scheduledDate`,
    ADD COLUMN `categoryId` INTEGER NOT NULL,
    ADD COLUMN `duedate` DATETIME(3) NULL,
    ADD COLUMN `frequency` VARCHAR(191) NULL,
    ADD COLUMN `priority` VARCHAR(191) NULL,
    ADD COLUMN `recurrenceRule` VARCHAR(191) NULL,
...
-- AddForeignKey
ALTER TABLE `MaintenanceSchedule` ADD CONSTRAINT `MaintenanceSchedule_categoryId_fkey` FOREIGN KEY (`categoryId`) REFERENCES `UtilityType`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
